...
void initializeDefaultScene( out STriangle triangles[12],  out SSphere spheres[3])
{
	...
	spheres[0].MaterialIdx = 3;

	spheres[1].Center = vec3(2.0, 1.0, 2.0);
	spheres[1].Radius = 1.0; 
	spheres[1].MaterialIdx = 4;
	
	spheres[2].Center = vec3(-1.0, 2.0, 2.0);
...
bool Raytrace(SRay ray, float start, float final, inout SIntersection intersect)
{
	...
	for (int i = 0; i < 3; i++) {
		...
		if (IntersectSphere(sphere, ray, start, final, test) && test < intersect.Time) {
			...
			intersect.LightCoeffs = vec4(0.0, 0.0, 0.50, 1);
			intersect.ReflectionCoef = 1.05;
			intersect.RefractionCoef = 1;
			intersect.MaterialType = MIRROR_REFLECTION;
			if (i == 1) {
				intersect.Color = materials[4].Color;
				intersect.LightCoeffs = materials[4].LightCoeffs;
				intersect.ReflectionCoef = materials[4].ReflectionCoef;
				intersect.RefractionCoef = materials[4].RefractionCoef;
				intersect.MaterialType = materials[4].MaterialType;
			} else {
				intersect.MaterialType = MIRROR_REFLECTION;
			}
			result = true;
...
{
	light.Position = vec3(0.0, 2.0, 0);

	vec4 lightCoefs = vec4(0.4,0.9,5.0,512.0);
	
	materials[0].Color = vec3(0.2, 0.2, 0.2);
	materials[0].LightCoeffs = vec4(lightCoefs);
	materials[0].ReflectionCoef = 0.5;
	materials[0].RefractionCoef = 1.0;
	materials[0].MaterialType = DIFFUSE;
	
	materials[1].Color = vec3(0, 1.0, 0);
	materials[1].LightCoeffs = vec4(lightCoefs);
	materials[1].ReflectionCoef = 0;
	materials[1].RefractionCoef = 1.0;
	materials[1].MaterialType = DIFFUSE;

	materials[2].Color = vec3(1.0, 0, 1.0);
		...
		if (Raytrace(ray, start, final, intersect))
		{
			switch(intersect.MaterialType)
			{
				...
				case MIRROR_REFLECTION:
				{
					...
					if (reflectRay.depth <= RAYTRACING_DEPTH) {
						push(reflectRay);
					} 
					break;
				}
				case REFRACTION:
				{
					...
					if (refractRay.depth <= RAYTRACING_DEPTH) {
						push(refractRay);
					}
					break;
				}
